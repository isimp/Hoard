<#
.SYNOPSIS
    Writes metadata-only copies of the assemblies Hoard compiles against into lib/.
...
# Every assembly listed in Hoard.csproj. Keep the two lists in step.
#
# assembly_utils is here for GetStableHashCode, which the ZDO key uses.
$sources = @(
    ...
    # ServerSync rewrites the connect-error screen (TextMeshPro), and the marking key checks
    # whether a text field has focus (TMP_InputField / InputField, EventSystem).
