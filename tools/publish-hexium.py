#!/usr/bin/env python3
"""Upload a package zip to Hexium and submit it as a new version.

Hexium describes its own API as "Thunderstore-compatible mod repository API"
(https://hexium.gg/api/openapi.json), and the endpoint names match Thunderstore's
experimental API one for one, so the request shapes here are Thunderstore's:

    1. POST /api/experimental/usermedia/initiate-upload/   -> a upload uuid + presigned part URLs
...
The one thing Hexium does not publish is the shape of its Authorization header — an
unauthenticated call answers `401 {"detail": "Authentication required."}` with no
WWW-Authenticate to read. So the first authenticated call tries `Bearer` (what Thunderstore
uses) and falls back to `Token` once, reporting which one the server accepted.

Usage:
    HEXIUM_TOKEN=... python3 tools/publish-hexium.py dist/Hoard-0.1.0.zip \\
        --team isimp --categories "Storage,Quality of Life,Open Source,Valheim 1.0"

    python3 tools/publish-hexium.py dist/Hoard-0.1.0.zip --check-only
"""
...
# What the site requires of a package, checked here so a bad zip fails in seconds
...
def call(path, payload, token, scheme):
    """POST JSON to Hexium. Returns (status, parsed body or raw text)."""
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "freelook-release/1.0",
        },
    )
    try:
...
def authenticated_call(path, payload, token, state):
    ...
    for scheme in schemes:
        status, body = call(path, payload, token, scheme)
...
def check(zip_path):
    """Validate the package against Hexium's stated rules. Returns the manifest."""
...
def publish(zip_path, manifest, team, categories, token):
    size = os.path.getsize(zip_path)
    filename = os.path.basename(zip_path)
    state = {"scheme": None}

    log(f"initiating upload of {filename} ({size:,} bytes)")
    ...
    authenticated_call(f"/api/experimental/usermedia/{uuid}/finish-upload/", {"parts": parts}, token, state)
    log("  upload finished")

    log(f"submitting as {team}/{manifest['name']} {manifest['version_number']}")
    result = authenticated_call(
    ...
    log(f"{BASE}/mods/{team}/{manifest['name']}")


def main():
    parser = argparse.ArgumentParser(description="Publish a package zip to Hexium.")
    parser.add_argument("zip", help="the package to upload")
    parser.add_argument("--team", help="the Hexium team to publish under (the package namespace)")
    parser.add_argument("--categories", default="", help="comma-separated category slugs")
    ...
    try:
        ...
        if not token:
            raise Failed(
                "HEXIUM_TOKEN is empty. Create an API token in your Hexium team settings and "
                "store it as the HEXIUM_TOKEN repository secret."
            )

        categories = [c.strip() for c in arguments.categories.split(",") if c.strip()]
        publish(arguments.zip, manifest, arguments.team, categories, token)
        return 0
